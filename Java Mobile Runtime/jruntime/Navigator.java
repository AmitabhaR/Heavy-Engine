      ...
      private boolean isRunning = false;
        ...
        public void start()
        {
            if (!isRunning)
            {
                ...
                {
                    makePath((Vector2) nav_points.elementAt(current_frame), (Vector2) nav_points.elementAt(current_frame + 1)); 
            ...
            if (current_frame < nav_points.size( ) && current_frame + 1 < nav_points.size())
                if (this.baseObject.obj_instance.img != null)
                    if (update_counter == total_updates)
                    {
                        ...
                        if (current_frame < nav_points.size( ) && current_frame + 1 < nav_points.size( )) makePath(((Vector2) nav_points.elementAt(current_frame)), ((Vector2) nav_points.elementAt(current_frame + 1)));
                        else stop();
                    }
                    else
                    {
                        ...
                        {
                            baseObject.pos_x = pos_x;
                            baseObject.pos_y = (int)(slope * (pos_x - ((Vector2) nav_points.elementAt(current_frame)).x) + ((Vector2) nav_points.elementAt(current_frame)).y);
                            pos_x += navigation_speed * ((deltaX < 0) ? -1 : (deltaX > 0) ? 1 : 0);
                        }
                        else
                        {
                            baseObject.pos_y = pos_y;
                            baseObject.pos_x = (int)(slope * (pos_y - ((Vector2) nav_points.elementAt(current_frame)).y) + ((Vector2) nav_points.elementAt(current_frame)).x);
